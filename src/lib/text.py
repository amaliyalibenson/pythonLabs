    ...
    while i < len(text):
        if text[i] == "-":
            if i > 0 and i < len(text) - 1:  # если он не с краю
                ...
                ):  # слева и справа буквы или цифры или двойной дефис
...
def count_freq(tokens: list[str]) -> dict[str, int]:
    ...
    sorted_res = sorted(
        res.items(), key=lambda x: (x[0], x[1])
    )  # items для словарей, превращает в список кортежей
...
def top_n(freq: dict[str, int], n: int = 5) -> list[tuple[str, int]]:
    ...
    """
    kolvo = list(
        count_freq(freq).items()
    )  # создаем с помощью айтемс список кортежей и делаем его полноценным списком (без "dict_items")
    return kolvo[:n]
